...
public class MysticalCardGame {
    ...
    public static void startDuel(SLL<Card> firstSorcererCards, SLL<Card> secondSorcererCards) {
        SLLNode<Card> findBestCard = firstSorcererCards.getFirst();
        int max = 0;
        SLLNode<Card> Bestcard = findBestCard;
        SLLNode<Card> PrevCard = findBestCard;
        SLLNode<Card> PrevPrevCard = findBestCard;
        for (int i = 0; i < firstSorcererCards.size(); i++) {
            Card c = (Card) findBestCard.element;
            int health = c.getHealth();
            int power = c.getMagicPower();
            if (health * power > max) {
                max = health * power;
                Bestcard = findBestCard;
                PrevPrevCard = PrevCard;
            }
            PrevCard = findBestCard;
            findBestCard = findBestCard.succ;
        }

        PrevPrevCard.succ = Bestcard.succ;
        Bestcard.succ = null;

        Card card = new Card(Bestcard.element.getType(), Bestcard.element.getHealth(), Bestcard.element.getMagicPower());


        SLLNode<Card> Findmiddle = secondSorcererCards.getFirst();
        for (int i = 0; i < (secondSorcererCards.size() / 2) + 1; i++) {
            Findmiddle = Findmiddle.succ;
            if (i == (secondSorcererCards.size() / 2) - 1) {
                secondSorcererCards.insertAfter(card, Findmiddle);
            }
