...
public class OrganizacijaTest {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < N; i++){
            ...
            hashMap.put(parts[3], Integer.parseInt(parts[2]));
            graph.addEdge(hashMap.get(parts[1]),hashMap.get(parts[3]), Integer.parseInt(parts[4]));
        }
        String president = sc.nextLine();
        int p = graph.prim(hashMap.get(president)).stream().mapToInt(Edge::getWeight).sum();
