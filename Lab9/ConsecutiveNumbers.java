...
class BTree<E> {
    ...
    public boolean connumbers() {
        return Consecutivenumbers(root);
    }

    private boolean Consecutivenumbers(BNode<E> node) {
        if (node == null){
            return true;
        }
        if (node.left == null && node.right == null){
            ...
            if ((node.left != null)) {
                if ((Integer) node.info + 2 == (Integer) node.left.info) {
                    return Consecutivenumbers(node.left);
                } else {
                    return false;
                }
            }
...
public class ConsecutiveNumbers {
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println(tree.connumbers());
