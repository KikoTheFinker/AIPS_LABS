...
public class Main {
    ...
    public static SalesPerson salesChampion(SalesPerson [] arr)
    {
        ...
        for (int i = 1; i < arr.length; i++){
            if (max.getTotal() < arr[i].getTotal()){
                max = arr[i];
            }
        }
    ...
    public static void table(SalesPerson [] arr)
    {
        ...
        for (int i = 0; i < arr.length; i++){
            ...
            for (int j = 0; j < arr[i].getQuarters().length; j++){
                System.out.print(arr[i].getQuarters()[j] + "   ");
            }
    ...
    public static void main(String[] args) {
        ...
        QuarterlySales[] p = new QuarterlySales[4];
        for(int i=0;i<n;i++)
        {
            String name = input.next();
            for (int k = 0; k < 4; k++) {
                int numSales = input.nextInt();
                int[] array = new int[numSales];
                for (int j = 0; j < numSales; j++) {

                    array[j] = input.nextInt();
                }
                p[k] = new QuarterlySales(numSales, array, k);

            }
            arr[i] = new SalesPerson(name,p);
        }
